...
public interface TeacherRepository extends JpaRepository<Teacher, Long> {
    ...
    @Query("SELECT DISTINCT t FROM Teacher t WHERE (t IN (SELECT DISTINCT t FROM Teacher t JOIN t.teacherSubjects ts WHERE s.subject.subjectName IN :subjectNames) OR :subjectNames IS NULL) AND (t IN (SELECT DISTINCT t FROM Teacher t JOIN t.teacherRegions tr WHERE tr.region.regionName IN :regionNames) OR :regionNames IS NULL) AND t.name LIKE %:teacherName%")
